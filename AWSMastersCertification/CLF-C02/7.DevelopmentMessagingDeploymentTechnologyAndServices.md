...
- **Queue:** A temporary repository for messages awaiting processing. Messages stay in the queue until the consumer is ready to process them, preventing any single component from becoming overloaded with requests.

#### Decoupling

- **Decoupling:** This approach ensures that different components of an application are not directly dependent on each other. Instead, they communicate through a queue that acts as an intermediary, allowing for asynchronous processing and improved fault tolerance.

#### Pull-Based Model

- **Pull-Based:** In SQS, consumers pull messages from the queue when they are ready to process the next message. This model allows for efficient message processing and prevents overloading the consumer.

#### Asynchronous Processing

- **Asynchronous Processing:** Messages are processed asynchronously, meaning that the producer does not wait for the consumer to finish processing. This allows for better performance and scalability.

#### Message Delivery

- **At-Least-Once Delivery:** With standard SQS queues, messages are guaranteed to be processed at least once, ensuring reliability.
