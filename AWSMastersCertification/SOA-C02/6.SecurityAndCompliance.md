...
### Key Differences Between Quota and Limit:
|**Feature**|**Quota**|**Limit**|
|---|---|---|
|**Definition**| Maximum usage allowance by AWS|	Threshold you define or monitor|
|**Scope**| Defined by AWS per service/account|	Set by the user for alerts/actions|
|**Modification**|Can request increases for many services|Usually static or custom-defined|
|**Monitoring**|Monitored passively via Service Quotas	|Monitored actively via CloudWatch/Trusted Advisor|
|**Example**|EC2 instance quota = 50 (default)|Alert at 45 EC2 instances|
...
## Protecting Logs within CloudTrail
### Recap: What is CloudTrail?
- **Purpose**: Tracks user activity in AWS.
- **Records**: Events for resource creation, modification, or deletion.
  - Examples: IAM users, S3 buckets, EC2 instances.
- **Default Log Retention**: 90 days of account activity in CloudTrail.
- **Optional**: Create a trail to store logs indefinitely in S3.

<br>

### Why Protect CloudTrail Logs?
CloudTrail logs record:
- **Who, When, What, and Where**.
- **Source IP**, request parameters, and responses for API calls.

#### Risks if Logs Are Not Protected:
- Logs could be modified, deleted, or read by unauthorized users to hide malicious activity.
  
#### Built-In Protections:
1. **Server-Side Encryption**: Enabled by default.
2. **Log File Integrity Validation**: Ensures logs haven’t been tampered with.

#### Manual Step:
- Secure the **S3 bucket** storing the logs.

<br>

### Encrypting CloudTrail Logs
- **Default Setting**: When creating a trail in the AWS console, **server-side KMS encryption** is enabled.
  - Allows you to control:
    - Who can encrypt/decrypt logs.
    - User access permissions to the key.

<br>

### What is Integrity Validation?
Integrity validation ensures:
- Detection of modified or deleted logs.

#### How It Works:
1. **Hash Creation**:
   - A hash is created for every log file.
2. **Digest File**:
   - Generated hourly, referencing hashes of all log files from the past hour.
   - Digitally signed with AWS's private key.
3. **Security**:
   - AWS claims it's computationally infeasible to modify/delete logs without detection.

<br>

### Best Practices for Securing CloudTrail Logs in S3
1. **Dedicated S3 Bucket**:
   - Use a separate bucket for CloudTrail logs.
2. **Bucket Policy**:
   - Grant CloudTrail permissions to write logs.
3. **Restrict Access**:
   - Limit read access to security or SysOps teams.
...
Amazon GuardDuty is a **threat detection service** that leverages machine learning to detect malicious activities in your AWS account. It monitors various AWS resources and behaviors to identify threats.

#### Key Detection Areas:
- **Unusual API calls**:
  - Detects API calls from known malicious IP addresses.
- **CloudTrail monitoring**:
  - Tracks attempts to disable CloudTrail logging.
- **Compromised S3 buckets**:
  - Identifies unauthorized access from malicious IP addresses.
...
- **Reconnaissance activities**:
  - Tracks port scanning and failed login attempts.
- **Compromised accounts**:
  - Detects unexpected behavior, such as unusual infrastructure launches.
...
#### 1. **Alerting**:
- Alerts appear in the GuardDuty dashboard.

#### 2. **Threat Intelligence Feeds**:
- Integrates feeds from sources like:
  - **Proofpoint**
  - **CrowdStrike**
  - **AWS Security**
...
- Displays findings in the dashboard with severity levels:
  - Low
  - Medium
  - High

#### Example:
GuardDuty might report a finding about a compromised EC2 instance communicating with a malicious IP address. This finding would include its priority and suggested actions.
...
1. **Threat Detection**:
   - Example: A compromised EC2 instance communicates with a malicious IP.
2. **Finding Generation**:
   - GuardDuty generates a finding, which appears in the dashboard.
3. **Response**:
   - Action is required, such as updating the security group to restrict access.
4. **Remediation**:
   - After restricting access, the malicious actor no longer has access.
...
1. **Enabling GuardDuty**:
   - Takes **7-14 days** to analyze your account and establish a baseline of normal behavior.
2. **Findings**:
   - Appear in the dashboard only when threats are detected.
3. **Free Trial**:
   - First **30 days are free**.
   - Charges apply after 30 days based on:
     - CloudTrail and S3 events.
     - DNS and VPC flow log data.
4. **Cost Management**:
   - Disable GuardDuty if only experimenting to avoid charges.
