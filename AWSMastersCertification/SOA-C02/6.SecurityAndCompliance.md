...
- **Key Features:**  
  - Centralized SSO management for AWS accounts, cloud apps, and SAML 2.0 applications.  
  - Integrates with **AWS Organizations** to streamline access and permissions across accounts and organizational units.  
  - Leverages **existing Active Directory (AD)** from AWS or corporate data centers.  
  - Facilitates **user and group provisioning** and management within the AWS console.  
...
2. **Cloud Applications:**  
   - Single-click SSO access for third-party apps like Dropbox, Slack, Atlassian tools, and Office 365.  
...
- **Purpose**: Restrict or allow access to AWS services, resources, and regions.  
- **Application**:  
  - Can be attached to an OU, an account, or specific users.  
  - Example: Prevent certain actions like changing billing options.  

#### **3. Policies in AWS Organizations**  
...
- **Security Hub**: Centralized security checks and alerts.  
- **IAM Identity Center**: Single Sign-On using Active Directory.  

**Management & Governance**:  
- **AWS Config**: Configuration tracking.  
- **Systems Manager**: Centralized EC2 management.  
- **AWS Service Catalog**: Approved IT service deployments.  
...
- **Centralized Payment**: All accounts use a single payment method.  
- **Monitor Usage**: View and manage costs across accounts.  
- **Optimize Costs**: Identify underutilized resources.  
