...
### Static Addressing
- The **administrator manually configures** the IPv6 address on a device’s interface.
- Example configuration (router interface):
  ```
  2001:db8:4:a::1/64
  ```
  - `2001:db8::/32` → documentation/test prefix (not routable on the public IPv6 internet).
  - `::1` → the device’s interface identifier.
  - `/64` → subnet size (standard for IPv6 LANs).
- To enable communication:
  - Assign a **static IPv6 address** to the router.
  - Assign a **static IPv6 address** on the workstation in the **same /64 network**.

### Example (Linux Workstation)
Static configuration using `ip` command:

```bash
sudo ip -6 addr add 2001:db8:4:a::10/64 dev eth0
sudo ip -6 route add default via 2001:db8:4:a::1
```

- `2001:db8:4:a::10/64` → workstation’s IPv6 address.
- `2001:db8:4:a::1` → router’s IPv6 address (default gateway).

### Key Point
- Both devices must share the **same network prefix (/64)** to communicate.
- Documentation prefix (`2001:db8::/32`) can be safely used for **labs, demos, and testing**, but not for public internet routing.
...
## IP Address Acquisition - SLAAC
SLAAC (Stateless Address Auto Configuration) allows devices to automatically configure their own IPv6 addresses without using DHCP.

When a router with IPv6 enabled is configured (for example, with the prefix `2001:db8:4:a::/64`), it periodically sends **Router Advertisement (RA)** messages using its link-local address.
These messages inform connected devices about the available IPv6 network prefix.

### Address Generation Process
Once a workstation receives an RA, it creates its own IPv6 address based on the advertised prefix.
The method for generating the **host portion (interface identifier)** depends on the operating system:
- **Windows**: Creates a **random 64-bit interface ID** for privacy.
- **macOS, Linux, and UNIX**: Use **Modified EUI-64** based on the device’s MAC address.

### Modified EUI-64 Method (Used on macOS/Linux/UNIX)
1. The device retrieves its **48-bit MAC address** (e.g., `0c:4d:e9:8a:62:1f`).
2. It splits the address in half and inserts `FF:FE` in the middle:

   ```
   0c:4d:e9:ff:fe:8a:62:1f
   ```
3. The **7th bit** of the first byte is inverted (from 0 to 1), forming the final **64-bit interface ID**.
4. The host portion is then combined with the network prefix to form the full IPv6 address:

   ```
   2001:db8:4:a:0e4d:e9ff:fe8a:621f
   ```

### Neighbor Advertisement
After configuring its address, the device sends a **Neighbor Advertisement (NA)** to inform other devices on the network that it now owns this IPv6 address.

### Privacy Considerations
Since EUI-64 embeds the device’s **MAC address** into the IPv6 address:
- It helps with **troubleshooting**, since the address reveals device identity.
- It can also expose **hardware information** (manufacturer and serial ID) to external networks.

To mitigate this, modern operating systems (including macOS) support **Privacy Extensions**, which generate **temporary randomized IPv6 addresses** instead of using the MAC-based EUI-64 identifier.

<br>

## IPv6 Address Acquisition - DHCP
The final method of obtaining an IPv6 address is by using **Dynamic Host Configuration Protocol (DHCP)**.

When using DHCP for IPv6 (DHCPv6), the process typically looks like this:

1. **Static IPv6 configuration on the router**
   - The network administrator assigns a static IPv6 address to the router.
   - A DHCP server is then configured and connected to the network.
2. **Client request**
   - A client sends out a DHCPv6 request indicating it needs an IPv6 address.
   - The DHCP server receives the request and provides an address from its pool.
3. **DHCPv6 advantages**
   - The administrator can easily **track which device has which IPv6 address** using the DHCP server logs or lease table.
   - The **host portion of the address can be simplified**, for example using `::10` instead of a long randomly generated 64-bit interface ID.
   - This makes IPv6 addressing easier to manage and read, especially in larger networks.
4. **Recommendation for enterprise networks**
   - In enterprise environments, DHCPv6 is preferred over **Stateless Address Autoconfiguration (SLAAC)**.
   - SLAAC is ideal for lab or small environments but can complicate troubleshooting since devices choose random interface identifiers.
   - DHCPv6 provides more **control, consistency, and visibility** in the network.

**Example scenario:**
- Router is configured with a static IPv6 address.
- DHCPv6 server assigns IPv6 addresses to clients like `2001:db8:4:AAAA::10`.
- Administrator can quickly identify and manage this address from the DHCP server, unlike SLAAC where addresses are randomized and harder to track.
...
## IPv6 Subnetting
Unlike IPv4 subnetting, **IPv6 subnetting is generally much simpler**. This is mainly because IPv6 uses **hexadecimal** representation, allowing us to avoid dealing with complex binary calculations.

### Learning Objectives for This Module
- **Review IPv4 subnet masks** to establish a foundation and contrast with IPv6.
- **Introduce IPv6 subnetting** concepts and how they differ from IPv4.
- **Subnet various IPv6 address spaces**, showing how they are allocated from ISPs down to enterprise networks.

### Typical IPv6 Allocation Hierarchy
1. **/32 prefix**
   - Commonly allocated to Internet Service Providers (ISPs).
   - This large block can be further subdivided for their customers.
2. **/48 prefix**
   - Usually allocated by ISPs to individual companies.
   - This allows organizations to further segment their address space.
3. **/56 prefix**
   - Often used by companies to allocate address space per site or location.
   - Provides flexibility and room to scale.
4. **/64 prefix**
   - Typically assigned to individual networks or VLANs.
   - This is the standard subnet size for most IPv6 networks.
...
In **IPv4**, an address is **32 bits** long and divided into **four octets**. For example:

```
192.168.10.10 with mask 255.255.255.192
```

If we write this in binary, we can clearly identify where the **network** and **host** portions of the address are.
- The **network prefix** in this example is the first **26 bits**.
- Using **CIDR notation**, we write this as:

```
192.168.10.10/26
```

In IPv4, we can **move the mask bit by bit** depending on network design needs, adjusting the number of hosts or subnets.

### IPv6 Subnetting Approach
In **IPv6**, the subnetting approach is different:
- Networks that contain **hosts (computers, servers, printers, etc.)** are almost always assigned a **/64** subnet mask.
- This is not mandatory but is the **recommended design**.
- Even though it may feel like a waste of billions of addresses (especially to those with an IPv4 mindset), **IPv6 was designed with this in mind**, and conservation is no longer a concern.

### IPv6 Address Structure with /64
| Portion              | Length (bits) | Description                    |
| -------------------- | ------------- | ------------------------------ |
| Network Prefix       | 64 bits       | Identifies the subnet          |
| Interface Identifier | 64 bits       | Uniquely identifies the device |

- Each **hextet** in an IPv6 address represents **16 bits**.
- **4 bits per hexadecimal digit** × **4 digits per hextet** = **16 bits**.
- First 4 hextets = `4 × 16 bits` = `64 bits` → this is the **network prefix**.
- The remaining 4 hextets = another `64 bits` → this is the **interface identifier**.
...
| Hextet | Value               | Bits | Portion              |
| ------ | ------------------- | ---- | -------------------- |
| 1      | 2001                | 16   | Network prefix       |
| 2      | 0db8                | 16   | Network prefix       |
| 3      | 0000                | 16   | Network prefix       |
| 4      | 0001                | 16   | Network prefix       |
| 5–8    | 0000:0000:0000:0001 | 64   | Interface Identifier |

**Key takeaway:**
When subnetting IPv6 networks, we **manipulate only the network prefix portion (first 64 bits)**. The interface identifier is typically auto-generated or assigned.
...
### IPv6 Network Example on a Router
Consider a router with two interfaces:

| Interface | Network Prefix          | Mask | Description                            |
| --------- | ----------------------- | ---- | -------------------------------------- |
| G0/0      | `2001:0DB8:0002:008D::` | /64  | First network segment                  |
| G0/1      | `2001:0DB8:0002:008E::` | /64  | Second network segment (unique subnet) |

Each interface uses a **unique /64 network**, and the router provides connectivity between them. This is the **typical design goal** in IPv6 deployments—networks on each router interface get a /64.

<br>

### How IPv6 Address Space Is Distributed

IPv6 address distribution starts at the **top level** with the **regional registries** under Internet Assigned Numbers Authority (IANA). IANA delegates large address blocks to regional registries:

| Regional Registry | Region Covered                       |
| ----------------- | ------------------------------------ |
| AFRINIC           | Africa                               |
| APNIC             | Asia Pacific, including Australia    |
| ARIN              | U.S., Canada, parts of the Caribbean |
| LACNIC            | Latin America and Caribbean          |
| RIPE NCC          | Europe, Middle East, Central Asia    |

These regional organizations then assign address blocks to **local ISPs**, which in turn distribute addresses to **customers**.

<br>

### ISP Allocation: /32 to /48 Subnetting
Suppose ARIN assigns an ISP the address block:

```
2001:0DB8::/32
```

- This is a **/32 prefix** (common for ISPs).
- The remaining 96 bits are below this /32.
- We don’t consider the **interface identifier** until we get down to /64 at the customer level.
- Between /32 and /64, we have space to **subnet**.

To make it easier to manage, the ISP decides to give each customer a **/48**.

| Prefix Length | Bits Available | Purpose                          |
| ------------- | -------------- | -------------------------------- |
| /32           | 32 bits        | ISP allocation from ARIN         |
| /48           | 48 bits        | Allocation to customers          |
| /64           | 64 bits        | End-network segments at customer |

<br>

### Subnetting the ISP’s /32 into /48s
We focus on **hextet 3** (the “pink area” between /32 and /64). By counting in hexadecimal, the ISP can allocate multiple /48 subnets.

| Customer | IPv6 Prefix           | CIDR | Hexadecimal Counter |
| -------- | --------------------- | ---- | ------------------- |
| 1        | `2001:0DB8:0000::/48` | /48  | 0000                |
| 2        | `2001:0DB8:0001::/48` | /48  | 0001                |
| 3        | `2001:0DB8:0002::/48` | /48  | 0002                |
| 4        | `2001:0DB8:0003::/48` | /48  | 0003                |
| 5        | `2001:0DB8:0004::/48` | /48  | 0004                |

The ISP can continue incrementing in hexadecimal (`0000` to `FFFF`) in this hextet, meaning:

```
2^16 = 65,536 /48 subnets
```

That means **one /32 allocation can support 65,536 customers**, each receiving a **/48**.

<br>

### Customer Level Subnetting
Once a customer receives a **/48**, they can further subnet their address space down to **/64** for individual networks.

For example, if Customer 1 receives:
...
| Subnet | IPv6 Prefix                | CIDR |
| ------ | -------------------------- | ---- |
| 1      | `2001:0DB8:0000:0000::/64` | /64  |
| 2      | `2001:0DB8:0000:0001::/64` | /64  |
| 3      | `2001:0DB8:0000:0002::/64` | /64  |
| ...    | ...                        | ...  |

Each **/64** provides **2⁶⁴ host addresses** (roughly 18 quintillion) for that single network segment.